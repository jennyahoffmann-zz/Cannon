/*!
 * cannonboard,js SWTPP team (fellmuth)
 * 
 * adaptes from:
...
function validSquare(square) {
  ...
  return (square.search(/^[a-j][0-9]+$/) !== -1);
}

function validPieceCode(code) {
  if (typeof code !== 'string') return false;
  return (code.search(/^[bBwWe1-9]*$/) !== -1);
}
...
function fenToObj(fen) {
  ...
  var position = {};
	...
	for (var r = BOARD_SIZE-1; r >= 0; r--) {
		var c = 0;
		var decoration = '';		
		if (chunks[r].length == 0) {
			for (var i =0; i < BOARD_SIZE; i++) {
				var square = COLUMNS[i] + (BOARD_SIZE -1 - r);
		        position[square] ='';
			}
		} else {
			for (var p = 0; p < chunks[r].length; p++) {
				if (chunks[r].charAt(p).search(/[0-9]/) != -1){
					for (var i =0; i < parseInt(chunks[r].charAt(p)); i++) {
						var square = COLUMNS[c] + (BOARD_SIZE -1 - r);
				        position[square] ='';
				        c++;
					}
				} else if (chunks[r].charAt(p) == 'e'){
					// TODO decoration
					//decoration = 'e';
				} else {
			        var square = COLUMNS[c] + (BOARD_SIZE -1 - r);
			        position[square] =chunks[r].charAt(p);
			        c++;
				}
			}
		}
...
window['CannonBoard'] = window['CannonBoard'] || function(containerElOrId, cfg) {
'use strict';
...
var MINIMUM_JQUERY_VERSION = '1.7.0',
  START_FEN = '/1w1w1w1w1w/1w1w1w1w1w/1w1w1w1w1w///b1b1b1b1b1/b1b1b1b1b1/b1b1b1b1b1/',
  ...
  if (typeof containerElOrId === 'string') {
    ...
    if (containerElOrId === '') {
      window.alert('Board Error 1001: ' +
        'The first argument to CannonBoard() cannot be an empty string.' +
        '\n\nExiting...');
      return false;
    }
...
function buildBoard(orientation) {
  ...
  for (var i = 0; i < BOARD_SIZE; i++) {
    ...
    for (var j = 0; j < BOARD_SIZE; j++) {
      ...
      if (cfg.showNotation === true) {
        // alpha notation
        if ((orientation === 'white' && row === 0) ||
            (orientation === 'black' && row === BOARD_SIZE-1)) {
        ...
        // numeric notation
        if (j === 0) {
          //TODO: row -1 s.t. number starts at 0
...
function paintSoldier(canvas, white, target) {
...
function paintCity(canvas, white, target) {
    var context = canvas.getContext('2d');
	if (white) {
		context.fillStyle = 'white';
		context.strokeStyle = 'white';
...
function buildPieceImgSrc(piece) {
	...
	switch(piece) {
		case 'w': paintSoldier(canvas, true); break;
		case 'b': paintSoldier(canvas, false); break;
		case 'W': paintCity(canvas, true); break;
		case 'B': paintCity(canvas, false); break;
		case 't': break;
		default: 
	}
...
/*
function buildPieceImgSrcChess(piece) {
...
}; // end window.CannonBoard

// expose util functions
window.CannonBoard.fenToObj = fenToObj;
window.CannonBoard.objToFen = objToFen;
