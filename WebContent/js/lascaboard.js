...
function validSquare(square) {
  ...
  return (square.search(/^[a-g][1-7]$/) !== -1);
}

function validPieceCode(code) {
  if (typeof code !== 'string') return false;
  return (code.search(/^[bBwW]*$/) !== -1);
}
...
function validFen(fen) {
  ...
  for (var i = 0; i < BOARD_SIZE; i++) {
	...
	if (figs.length > BOARD_SIZE || chunks[i].search(/[^,bBwW]/) !== -1) {
	      return false;
	    }
...
var MINIMUM_JQUERY_VERSION = '1.7.0',
  START_FEN = 'b,,b,,b,,b/,b,,b,,b,/b,,b,,b,,b/,,,,,,/w,,w,,w,,w/,w,,w,,w,/w,,w,,w,,w',
    ...
    for (var j = 0; j < BOARD_SIZE; j++) {
      ...
      if (cfg.showNotation === true) {
        ...
        if ((orientation === 'white' && row === 1) ||
            (orientation === 'black' && row === BOARD_SIZE)) {
...
function paintChip(canvas, offset, white, boss) {
    var context = canvas.getContext('2d');
	context.beginPath();
	context.moveTo(10, offset);
	context.bezierCurveTo(10, offset+10,SQUARE_SIZE-10, offset+10, SQUARE_SIZE-10,offset);
	context.bezierCurveTo(SQUARE_SIZE-10, offset-10,10, offset-10, 10,offset);
	if (white) {
	...
	if (boss) {
		...
		context.moveTo(SQUARE_SIZE/3,offset-3);
		context.lineTo(SQUARE_SIZE-SQUARE_SIZE/3,offset+3);
		context.moveTo(SQUARE_SIZE/3,offset+3);
		context.lineTo(SQUARE_SIZE-SQUARE_SIZE/3,offset-3);
		context.lineWidth = 2;
...
function buildPieceImgSrc(piece) {
	...
	for(var i=pieces.length-1; i >= 0; i--) {
		...
		switch(pieces[i]) {
			case 'b': paintChip(canvas, offset, false, false); break;
			case 'B': paintChip(canvas, offset, false, true); break;
			case 'w': paintChip(canvas, offset, true, false); break;
			case 'W': paintChip(canvas, offset, true, true); break;
			default: 
		}
		if (pieces.length < 9 && (pieces.length * 9) < SQUARE_SIZE) {
			offset -= 8;
		} else {
			offset -= 4;
		}
